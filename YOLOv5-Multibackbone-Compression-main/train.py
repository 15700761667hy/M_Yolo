...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        with torch_distributed_zero_first(LOCAL_RANK):
            weights = attempt_download(weights)  # download if not found locally 加载预训练模型
        ckpt = torch.load(weights, map_location=device)  # load checkpoint加载checkpoint
        #取模型
    ...
    g0, g1, g2 = [], [], []  # optimizer parameter groups 权重和偏置
    ...
    if opt.adam:
        optimizer = Adam(g0, lr=hyp['lr0'], betas=(hyp['momentum'], 0.999))  # adjust beta1 to momentum 动量
    else:
    ...
    if pretrained:
        ...
        if ckpt['optimizer'] is not None:
            optimizer.load_state_dict(ckpt['optimizer']) #加载ckpt文件
        ...
        if epochs < start_epoch:
            ...
            epochs += ckpt['epoch']  # finetune additional epochs 调整附加的epochs

        del ckpt, csd

    # Image sizes
    gs = max(int(model.stride.max()), 32)  # grid size (max stride) 网格大小，最大步长gs
    nl = model.model[-1].nl  # number of detection layers (used for scaling hyp['obj']) 检测层的数量
    ...
    if cuda and RANK == -1 and torch.cuda.device_count() > 1:
        ...
        model = torch.nn.DataParallel(model) #分布式

    # SyncBatchNorm
    if opt.sync_bn and cuda and RANK != -1:# 综合归一化
    ...
    mlc = int(np.concatenate(dataset.labels, 0)[:, 0].max())  # max label class 最大标签数量
    nb = len(train_loader)  # number of batches  batch_size 是一次训练的所取的样本数。batches 就是一个epoch中分为多少个batch
    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            ...
            if not opt.noautoanchor:
                check_anchors(dataset, model=model, thr=hyp['anchor_t'], imgsz=imgsz) #检查anchor 并更新anchor
            model.half().float()  # pre-reduce anchor precision

        callbacks.run('on_pretrain_routine_end')

    # DDP mode
    if cuda and RANK != -1:#也就是说RANK== 0  的时候，进行分布式训练
